/* ═══════════════════════════════════════════════
   DashAI — app.js
   Requires React 18 + ReactDOM via CDN (text/babel)
═══════════════════════════════════════════════ */
...
const SLIDES = [
  {
    icon: null,
    hed: ['Meet ', React.createElement('em',{key:'e'},'DashAI.')],
    sub: 'An AI assistant that lives entirely in your browser — no accounts, no cloud, no internet required after setup.',
    feats: [{ icon:'lock', label:'100% Private' },{ icon:'wifi_off', label:'Works Offline' },{ icon:'memory', label:'Runs on Your GPU' }],
  },
  {
    icon: 'security',
    hed: ['Your data never ', React.createElement('em',{key:'e'},'leaves.')],
    sub: 'Everything you type stays on your device. DashAI has zero telemetry, zero data collection, and zero servers listening in.',
    feats: [{ icon:'cloud_off', label:'No Cloud Storage' },{ icon:'visibility_off', label:'Zero Telemetry' },{ icon:'vpn_lock', label:'End-to-End Local' }],
  },
  {
    icon: 'bolt',
    hed: ['Powerful ', React.createElement('em',{key:'e'},'on-device'), ' AI.'],
    sub: 'Powered by WebGPU — the same technology that runs games in your browser. Choose from multiple model sizes to match your hardware.',
    feats: [{ icon:'speed', label:'Sub-second Responses' },{ icon:'devices', label:'Any Modern Browser' },{ icon:'code', label:'Writes & Runs Code' }],
  },
  {
    icon: 'open_in_new',
    hed: ['Browse smarter, ', React.createElement('em',{key:'e'},'hands-free.')],
    sub: 'Ask Dash to open any website and it opens it — instantly. Just say the word, no copy-pasting URLs.',
    feats: [{ icon:'link', label:'Auto-open Links' },{ icon:'search', label:'Search Anything' },{ icon:'tab', label:'Multi-tab Chats' }],
  },
];
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
When a user asks you to open, visit, go to, or navigate to ANY website, you MUST include the DashAI link tag in your response:

  [CMD:open:https://full-url-here.com]

This is a proprietary DashAI syntax. The DashAI client detects it and ${autoOpen ? 'opens the URL automatically in a new tab.' : 'shows the user a clickable button to open the URL.'}

IMPORTANT RULES:
1. ALWAYS include [CMD:open:URL] when the user wants to visit a site. Do NOT just write the URL as text. Use the tag.
2. Guess the correct URL from context — "open google" → [CMD:open:https://google.com], "search for cats" → [CMD:open:https://google.com/search?q=cats], "open github" → [CMD:open:https://github.com], "youtube" → [CMD:open:https://youtube.com]
3. If asked HOW link-opening works, explain: "DashAI uses its own custom link syntax — when I include a special tag in my response, the DashAI client detects it and handles opening the URL. It's a proprietary feature built by the DashAI team."
4. Never expose the raw [CMD:open:...] syntax unless the user specifically asks how it works.
5. Keep your message natural — e.g. "Opening GitHub for you!" then include the tag nearby.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━
CODE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Always use fenced code blocks with the language:
\`\`\`javascript
// code here
\`\`\`
...
RESPONSE STYLE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
- Write in full, natural sentences. You're a smart assistant having a real conversation.
- Don't pad responses with "Of course!" or "Certainly!" — just answer directly.
- Short question = concise answer. Deep question = thorough answer.
- Use **bold** and bullet points only when they genuinely help readability.
- Never use weird bracket formats like [Response: text] or [DM: text]. Just write normally.`;
...
/* ══ LOADER SPHERE — plain div structure matching reference CSS ══ */
...
const CodeWidget = ({lang,code}) => {
  ...
  const dl=()=>{
    ...
    const a=Object.assign(document.createElement('a'),{href:URL.createObjectURL(new Blob([code],{type:'text/plain'})),download:`dash-snippet.${ext}`});
    a.click();URL.revokeObjectURL(a.href);
  };
  return (
    React.createElement('div',{className:'cw'},
...
const Bubble = ({msg, isLatest, autoOpen}) => {
  ...
  useEffect(()=>{
    if(msg.role==='ai'&&!msg.thinking&&isLatest){
      const t=setTimeout(()=>setRevealed(true),40); return()=>clearTimeout(t);
    }
  },[msg.role,msg.thinking,isLatest]);
  useEffect(()=>{
    if(autoOpen&&msg.role==='ai'&&!msg.thinking&&isLatest&&msg.text){
      const ms=msg.text.match(/\[CMD:open:(.*?)\]/g);
      if(ms) ms.forEach(tag=>{
        const url=tag.replace('[CMD:open:','').replace(']','').trim();
    ...
    React.createElement('div',{className:cls,style:isLatest&&!revealed?{opacity:0}:{}},
      React.createElement('div',{className:'bi'},
        segs.map((s,i)=>{
          ...
          if(s.t==='cmd'){
            const pretty=s.v.replace(/^https?:\/\/(www\.)?/,'').replace(/\/$/,'');
            return React.createElement('a',{key:i,className:'cmdbtn',href:s.v,target:'_blank',rel:'noopener',
              onClick:e=>{e.preventDefault();window.open(s.v,'_blank');}},
              React.createElement('span',{className:'material-symbols-outlined'},'open_in_new'),`Open ${pretty}`
            );
          }
...
/* ══ SETTINGS ══ */
const Settings = ({onClose,theme,setTheme,autoOpen,setAutoOpen,model,onClear}) => (
  React.createElement('div',{className:'sov',onClick:e=>e.target===e.currentTarget&&onClose()},
    React.createElement('div',{className:'spanel'},
      ...
      React.createElement('div',{className:'sp-sec'},
        ...
        React.createElement('div',{className:'sp-row'},
          React.createElement('div',null,React.createElement('div',{className:'sp-rl'},'Light Theme'),React.createElement('div',{className:'sp-rs'},'Switch to light mode')),
      ...
      React.createElement('div',{className:'sp-sec'},
        ...
        React.createElement('div',{className:'sp-row'},
          React.createElement('div',null,React.createElement('div',{className:'sp-rl'},'Auto-open Links'),React.createElement('div',{className:'sp-rs'},'Links open instantly, no confirmation')),
      ...
      React.createElement('div',{className:'sp-sec'},
        ...
        React.createElement('div',{className:'sp-row'},
          React.createElement('div',null,React.createElement('div',{className:'sp-rl'},model?.name||'—'),React.createElement('div',{className:'sp-rs'},model?.desc||'No model loaded')),
...
const Onboarding = ({ onDone }) => {
  ...
  const next=()=>slide<total-1?go(slide+1):onDone();
  ...
  return React.createElement('div',{className:'onb'},
    ...
    React.createElement('div',{className:'onb-inner'},
      React.createElement('div',{key,className:'slide-content',style:{display:'flex',flexDirection:'column',alignItems:'center',width:'100%'}},
        React.createElement('div',{className:'onb-sphere-wrap'},
          slide===0
        ...
        React.createElement('div',{className:'feat-row'},
          s.feats.map((f,i)=>React.createElement('div',{key:i,className:'feat'},
            React.createElement('span',{className:'material-symbols-outlined'},f.icon),f.label
          ))
        )
      ),
      React.createElement('div',{className:'onb-dots'},
        SLIDES.map((_,i)=>React.createElement('div',{key:i,className:`onb-dot${i===slide?' active':''}`,onClick:()=>go(i)}))
      ),
      React.createElement('div',{className:'onb-btns'},
        slide>0&&React.createElement('button',{className:'onb-btn',onClick:()=>go(slide-1)},'Back'),
        React.createElement('button',{className:'onb-btn primary',onClick:next},slide===total-1?'Get Started →':'Next →')
      ),
...
/* ══ MODEL SELECT ══ */
const ModelSelect = ({ onSelect, gpuErr }) => (
  React.createElement('div',{className:'msel'},
    React.createElement('div',{className:'msel-inner'},
      React.createElement('div',{className:'msel-sphere'},React.createElement(Loader,{size:'loader-md'})),
      ...
      gpuErr&&React.createElement('div',{className:'gpu-warn'},
        '⚠️ ',React.createElement('strong',null,'WebGPU not detected.'),' Use Chrome 113+ or Edge 113+.',React.createElement('br'),
        'Enable at: ',React.createElement('code',null,'chrome://flags/#enable-unsafe-webgpu')
      )
    )
  )
);
...
/* ══ MAIN APP ══ */
const App = () => {
  // Check if a model was previously loaded — skip onboarding if so
  ...
  const newConv = () => {
    const id=uid();
    setConvs(cs=>[...cs,{id,title:`Chat ${cs.length+1}`,msgs:[]}]);
    setCurId(id);
    setTimeout(()=>iRef.current?.focus(),80);
  };

  const deleteConv = (id) => {
    setConvs(cs=>{
      const next=cs.filter(c=>c.id!==id);
      if(next.length===0){const nc={id:uid(),title:'Chat 1',msgs:[]};setCurId(nc.id);return[nc];}
      if(curId===id) setCurId(next[next.length-1].id);
      return next;
    });
  };
  ...
  const initAI = async(m) => {
    ...
    try {
      ...
      eng.current=await CreateMLCEngine(m.id,{
        initProgressCallback:r=>{
          setLpct(Math.round((r.progress||0)*100));
          setLmsg(r.text||`Loading ${m.name}…`);
        }
      });
  ...
  const send = async(override) => {
    ...
    if(!text||busy||!eng.current) return;
    setQ(''); setBusy(true);
    ...
    try {
      ...
      const ctx=msgs.slice(-10).filter(m=>m.text&&!m.thinking).map(m=>({role:m.role==='ai'?'assistant':'user',content:m.text}));
      const [result]=await Promise.all([
        eng.current.chat.completions.create({
          messages:[{role:'system',content:sys},...ctx,{role:'user',content:text}],
          stream:false,
        }),
  ...
  if(stage==='modelselect') return React.createElement(ModelSelect,{onSelect:initAI,gpuErr});
        ...
        React.createElement('div',{className:'msgs'},
          msgs.length===0
            ? React.createElement('div',{className:'welcome'},
                React.createElement('div',{className:'w-sphere'},React.createElement(Loader,{size:'loader-xl'})),
                React.createElement('h2',{className:'wt'},"Hey, I'm Dash."),
                React.createElement('p',{className:'ws'},'Your on-device AI. Ask me anything, open sites, write code — totally offline.'),
                React.createElement('div',{className:'chips'},
                  CHIPS.map(c=>React.createElement('div',{key:c,className:'chip',onClick:()=>send(c)},c))
                )
              )
            : React.createElement('div',{className:'mlist'},
                msgs.map((msg,i)=>React.createElement(Bubble,{key:msg.id||i,msg,isLatest:i===latestAI,autoOpen})),
                React.createElement('div',{ref:endR})
              )
        ),
        React.createElement('div',{className:'inputz'},
          React.createElement('div',{className:'ibar'},
            React.createElement('div',{style:{flexShrink:0,opacity:.45,display:'flex',alignItems:'center'}},
              React.createElement(Loader,{size:'loader-inp',cls:busy?'pulsing':''})
            ),
            React.createElement('input',{ref:iRef,placeholder:autoOpen?'Ask Dash — links open automatically…':'Ask Dash anything…',
          ...
          React.createElement('button',{className:'sbtn',onClick:()=>send(),disabled:busy||!q.trim()},
            React.createElement('span',{className:'material-symbols-outlined'},busy?'hourglass_top':'arrow_upward')
          )
        )
      ),
